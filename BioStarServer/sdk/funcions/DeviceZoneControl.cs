    ...
    public class DeviceZoneControl : FunctionModule
    {
        ...
        void print(ref BS2DeviceZoneAGEntranceLimit data, int Indented)
        {
            ...
            for (int i = 0; i < data.numAGEntranceLimit; i++)
            {
                ...
                Console.WriteLine(preIndented + "   [{0}]accessGroupID: {1}", i, ToString(data.accessGroupID, BS2Envirionment.BS2_MAX_ENTRANCE_LIMIT_PER_ZONE*i, data.numEntry[i]));
                Console.WriteLine(preIndented + "   [{0}]maxEntry: {1}", i, ToString(data.maxEntry, BS2Envirionment.BS2_MAX_ENTRANCE_LIMIT_PER_ZONE * i, data.numEntry[i]));
        ...
        {            
            ...
            Console.WriteLine("Enter listen port number of master device [{0}(default)]", (UInt16)BS2Envirionment.BS2_TCP_DEVICE_ZONE_MASTER_PORT_DEFAULT);
            Console.Write(">>>> ");
            config.listenPort = Util.GetInput((UInt16)BS2Envirionment.BS2_TCP_DEVICE_ZONE_MASTER_PORT_DEFAULT); ;
                ...
                case BS2_DEVICE_ZONE_NODE_TYPE.BS2_DEVICE_ZONE_NODE_TYPE_MASTER:
                    {  
                        ...
                        else
                        {
                            ...
                            Array.Clear(elm.name, 0, BS2Envirionment.BS2_MAX_ZONE_NAME_LEN);
                        ...
                        for (int idxEntranceLimitItem = 0; idxEntranceLimitItem < elm.numEntranceLimit; ++idxEntranceLimitItem)
                        {
                            ...
                            Console.WriteLine("How many entrance should you limit {0} th entrance limit? [0: ~ 253, {1}: Access Group]", idxEntranceLimitItem, BS2Envirionment.BS2_ENTRY_COUNT_FOR_ACCESS_GROUP_ENTRANCE_LIMIT);
                            Console.Write(">>>> ");
                            ...
                            unchecked
                            {
                                ...
                                bAGEL = elm.maxEntry[idxEntranceLimitItem] == (byte)BS2Envirionment.BS2_ENTRY_COUNT_FOR_ACCESS_GROUP_ENTRANCE_LIMIT;
                            }
                            if (bAGEL)
                            {
                                ...
                                Console.WriteLine("How many access group item do you want to set? [1(default)~{0}]", BS2Envirionment.BS2_MAX_ACCESS_GROUP_ENTRANCE_LIMIT_PER_ENTRACE_LIMIT);
                                Console.Write(">>>> ");
                                AGEL.numEntry[AGEL.numAGEntranceLimit] = Util.GetInput(1);
                                if (AGEL.numEntry[AGEL.numAGEntranceLimit] == 0 || AGEL.numEntry[AGEL.numAGEntranceLimit] > BS2Envirionment.BS2_MAX_ACCESS_GROUP_ENTRANCE_LIMIT_PER_ENTRACE_LIMIT)
                                {
                                ...
                                for (int idxAG = 0; idxAG < AGEL.numEntry[AGEL.numAGEntranceLimit]; ++idxAG)
                                {
                                    int AGID = 0;
                                    int entry = 0;
                                    Console.WriteLine("Enter the ID of the access group [1~, {0}: otherwise Access Group ID]", BS2Envirionment.BS2_OTHERWISE_ACCESS_GROUP_ID);
                                    Console.Write(">>>> ");
                                    AGID = Util.GetInput();
                                    ...
                                    Console.WriteLine("How many entrance should you limit [0(default): ~ 253, {0}: No Limit]", BS2Envirionment.BS2_ENTRY_COUNT_NO_LIMIT);
                                    Console.Write(">>>> ");
                                    entry = Util.GetInput();

                                    AGEL.accessGroupID[BS2Envirionment.BS2_MAX_ACCESS_GROUP_ENTRANCE_LIMIT_PER_ENTRACE_LIMIT * AGEL.numAGEntranceLimit + idxAG] = (UInt32)AGID;
                                    AGEL.maxEntry[BS2Envirionment.BS2_MAX_ACCESS_GROUP_ENTRANCE_LIMIT_PER_ENTRACE_LIMIT * AGEL.numAGEntranceLimit + idxAG] = (UInt16)entry;
                                }
                        ...
                        Console.WriteLine("Enter the ID of the readers which are used as member of this entrance limit zone: [ID_1,ID_2 ...] (Max:{0})", BS2Envirionment.BS2_MAX_READERS_PER_DEVICE_ZONE_ENTRANCE_LIMIT);
                        Console.Write(">>>> ");
                        ...
                        foreach (string readerID in readerIDs)
                        {
                            if (readerID.Length > 0)
                            {
                                ...
                                {
                                    if (elm.numReaders + 1 >= BS2Envirionment.BS2_MAX_READERS_PER_DEVICE_ZONE_ENTRANCE_LIMIT)
                                    {
                        ...
                        Console.WriteLine("How many alarms for this entracne limit zone do you want to set? [0(default)-{0}]", BS2Envirionment.BS2_MAX_DEVICE_ZONE_ENTRANCE_LIMIT_ALARM_ACTION);
                        Console.Write(">>>> ");
                        ...
                        Console.WriteLine("Enter the ID of the access group which can bypass this entrace limit zone: [ID_1,ID_2 ...] (Max:{0})", BS2Envirionment.BS2_MAX_BYPASS_GROUPS_PER_DEVICE_ZONE_ENTRANCE_LIMIT);
                        Console.Write(">>>> ");
                        ...
                        foreach (string accessGroupID in accessGroupIDs)
                        {
                            if (accessGroupID.Length > 0)
                            {
                                ...
                                {
                                    if (elm.numBypassGroups + 1 >= BS2Envirionment.BS2_MAX_BYPASS_GROUPS_PER_DEVICE_ZONE_ENTRANCE_LIMIT)
                                    {
                                        Console.WriteLine("[Warning] The count of bypass access group should less than {0}.", BS2Envirionment.BS2_MAX_BYPASS_GROUPS_PER_DEVICE_ZONE_ENTRANCE_LIMIT);
                                        break;
                                    }
                ...
                case BS2_DEVICE_ZONE_NODE_TYPE.BS2_DEVICE_ZONE_NODE_TYPE_MEMBER:
                    {
                        ...
                        Array.Clear(elm.masterIP, 0, BS2Envirionment.BS2_IPV4_ADDR_SIZE);
                        Array.Copy(deviceIpAddressArray, elm.masterIP, deviceIpAddressArray.Length);

                        Console.WriteLine("Enter the Port Number of the master device of entrance limit? : default[{0}]", BS2Envirionment.BS2_TCP_DEVICE_ZONE_MASTER_PORT_DEFAULT);
                        Console.Write(">>>> ");
                        elm.masterPort = Util.GetInput((UInt16)BS2Envirionment.BS2_TCP_DEVICE_ZONE_MASTER_PORT_DEFAULT);
                ...
                case BS2_DEVICE_ZONE_NODE_TYPE.BS2_DEVICE_ZONE_NODE_TYPE_MASTER:
                    {
                        ...
                        Console.WriteLine("Enter the ID of the readers which are used as member of this fire alarm zone: [ID_1,ID_2 ...] (Max:{0})", BS2Envirionment.BS2_MAX_READERS_PER_DEVICE_ZONE_ENTRANCE_LIMIT);
                        Console.Write(">>>> ");
                        string[] readerIDs = Console.ReadLine().Split(delimiterChars);
                        ...
                        Console.WriteLine("How many alarms for this fire alarm zone do you want to set? [0(default)-{0}]", BS2Envirionment.BS2_MAX_DEVICE_ZONE_ENTRANCE_LIMIT_ALARM_ACTION);
                        Console.Write(">>>> ");
                        elm.numAlarm = Util.GetInput(0);
                        BS2SoundAction sound = Util.AllocateStructure<BS2SoundAction>();
                ...
                case BS2_DEVICE_ZONE_NODE_TYPE.BS2_DEVICE_ZONE_NODE_TYPE_MEMBER:
                    {
                        ...
                        Console.WriteLine("Enter the Port Number of the master device of fire alarm zone? : default[{0}]", BS2Envirionment.BS2_TCP_DEVICE_ZONE_MASTER_PORT_DEFAULT);
                        Console.Write(">>>> ");
                        elm.masterPort = Util.GetInput((UInt16)BS2Envirionment.BS2_TCP_DEVICE_ZONE_MASTER_PORT_DEFAULT);

                        
                        Console.WriteLine("How many fire sensors for member of this fire alarm zone do you want to set? [1(default)-{0}]", BS2Envirionment.BS2_MAX_FIRE_SENSORS_PER_DEVICE_ZONE_FIRE_ALARM_MEMBER);
                        Console.Write(">>>> ");
                        ...
                        foreach (string doorID in doorIDs)
                        {
                            if (doorID.Length > 0)
                            {
                                ...
                                if (doorID == "-1" || UInt32.TryParse(doorID, out item))
                                {
                                    ...
                                    if (elm.numDoors + 1 >= BS2Envirionment.BS2_MAX_DOORS_PER_DEVICE_ZONE_FIRE_ALARM_MEMBER)
                                    {
                                        Console.WriteLine("[Warning] The count of doorID should less than {0}.", BS2Envirionment.BS2_MAX_DOORS_PER_DEVICE_ZONE_FIRE_ALARM_MEMBER);
                                        break;
                                    }
