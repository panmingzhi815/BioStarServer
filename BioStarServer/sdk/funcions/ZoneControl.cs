...
namespace Suprema
{
    public class ZoneControl : FunctionModule
    {
        ...
        void setIntrusionAlarmZone(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            Array.Clear(zoneBlob[0].IntrusionAlarmZone.name, 0, BS2Envirionment.BS2_MAX_ZONE_NAME_LEN);
        ...
        void setInterlockZone(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            if(amount > BS2Envirionment.BS2_MAX_INTERLOCK_ZONE)
            {
            ...
            for (int idx = 0; idx < amount; ++idx)
            {
                ...
                {
                    byte[] zoneNameArray = Encoding.UTF8.GetBytes(zoneName);
                    Array.Clear(zoneBlob[idx].InterlockZone.name, 0, BS2Envirionment.BS2_MAX_ZONE_NAME_LEN);
                ...
                Console.WriteLine("  How many doors do you want to add to the interlock zone?[2(Default/Min), ... , {0}(Max)", BS2Envirionment.BS2_MAX_DOORS_IN_INTERLOCK_ZONE);
                Console.Write("  >>>> ");
                zoneBlob[idx].InterlockZone.numDoors = Util.GetInput(2);
                if (zoneBlob[idx].InterlockZone.numDoors > BS2Envirionment.BS2_MAX_DOORS_IN_INTERLOCK_ZONE)
                {
                ...
                Console.WriteLine("  How many inputs do you want to set to the interlock zone?[0(Default), ... , {0}(Max)", BS2Envirionment.BS2_MAX_INPUTS_IN_INTERLOCK_ZONE);
                Console.Write("  >>>> ");
                zoneBlob[idx].InterlockZone.numInputs = Util.GetInput(0);
                if (zoneBlob[idx].InterlockZone.numInputs > BS2Envirionment.BS2_MAX_INPUTS_IN_INTERLOCK_ZONE)
                {
                    Console.WriteLine("   You cannot add more than {0} doors to the Interlock zone.", BS2Envirionment.BS2_MAX_INPUTS_IN_INTERLOCK_ZONE);
                    return;
                }
                ...
                Console.WriteLine("  How many outputs do you want to set to the interlock zone?[0(Default), ... , {0}(Max)", BS2Envirionment.BS2_MAX_OUTPUTS_IN_INTERLOCK_ZONE);
                Console.Write("  >>>> ");
                zoneBlob[idx].InterlockZone.numOutputs = Util.GetInput(0);
                if (zoneBlob[idx].InterlockZone.numOutputs > BS2Envirionment.BS2_MAX_OUTPUTS_IN_INTERLOCK_ZONE)
                {
                    Console.WriteLine("  You cannot add more than {0} doors to the Interlock zone.", BS2Envirionment.BS2_MAX_OUTPUTS_IN_INTERLOCK_ZONE);
                    return;
                }
