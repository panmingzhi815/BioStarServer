...
namespace Suprema
{
    public class CardControl : FunctionModule
    {
        ...
        public void writeCard(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            if(Convert.ToBoolean(deviceInfo.pinSupported))
            {
                ...
                if(!Util.IsNo())
                {
                    ...
                    Marshal.Copy(pinCode, smartCard.credentials.pin, 0, BS2Envirionment.BS2_PIN_HASH_SIZE);
                    Marshal.FreeHGlobal(pinCode);
                }
            }
            ...
            {
                Console.WriteLine("How many fingerprint templates do you want to set? [0(default)-{0}]", BS2Envirionment.BS2_SMART_CARD_MAX_TEMPLATE_COUNT);
                Console.Write(">>>> ");
            ...
            Array.Clear(smartCard.credentials.templateData, 0, BS2Envirionment.BS2_SMART_CARD_MAX_TEMPLATE_COUNT * BS2Envirionment.BS2_FINGER_TEMPLATE_SIZE);
            if(smartCard.header.numOfTemplate > 0)
            {
                Console.WriteLine("Enter the size of template which you want to set. [{0}(default)-{1}]", BS2Envirionment.BS2_SMART_CARD_MIN_TEMPLATE_SIZE, BS2Envirionment.BS2_FINGER_TEMPLATE_SIZE);
                Console.Write(">>>> ");
                smartCard.header.templateSize = Util.GetInput((UInt16)BS2Envirionment.BS2_SMART_CARD_MIN_TEMPLATE_SIZE);
            ...
            {
                smartCard.header.templateSize = (UInt16)BS2Envirionment.BS2_SMART_CARD_MIN_TEMPLATE_SIZE;
            ...
            if (cardIDStr.Length > BS2Envirionment.BS2_CARD_DATA_SIZE)
            {
                Console.WriteLine("Card id should less than {0} words.", BS2Envirionment.BS2_CARD_DATA_SIZE);
                return;
            }
            ...
            else
            {
                ...
                Array.Clear(smartCard.cardID, 0, BS2Envirionment.BS2_CARD_DATA_SIZE);

                for (int idx = 0; idx < cardIDArray.Length; ++idx)
                {
                    smartCard.cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - idx - 1] = cardIDArray[idx];
                }
            }
            ...
            if ((BS2CardTypeEnum)smartCard.header.cardType == BS2CardTypeEnum.ACCESS)
            {
                ...
                int idx = 0;
                Array.Clear(smartCard.accessOnData.accessGroupID, 0, BS2Envirionment.BS2_SMART_CARD_MAX_ACCESS_GROUP_COUNT);
                ...
                foreach (string accessGroupID in accessGroupIDs)
                {
                    if (accessGroupID.Length > 0)
                    {
                        ...
                        if (UInt16.TryParse(accessGroupID, out item))
                        {
                            ...
                            if (idx >= BS2Envirionment.BS2_SMART_CARD_MAX_ACCESS_GROUP_COUNT)
                            {
                                break;
                            }
                        }
                ...
                else if (numBlacklist > 0)
                {
                    ...
                    for (int idx = 0; idx < numBlacklist; ++idx)
                    {
                        Console.WriteLine("[{0:000}] ==> Blacklist issueCount[{0}] cardID[{1}-{2}-{3}-{4}-{5}-{6}-{7}-{8}]",
                                            ...
                                            blackList[idx].cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 1],
                                            blackList[idx].cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 2],
                                            blackList[idx].cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 3],
                                            blackList[idx].cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 4],
                                            blackList[idx].cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 5],
                                            blackList[idx].cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 6],
                                            blackList[idx].cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 7],
                                            blackList[idx].cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 8]);
                    }
                    Console.WriteLine("+----------------------------------------------------------------------------------------------------------+");
        ...
        void print(IntPtr sdkContext, BS2SmartCardData smartCard)
        {
            ...
            {
                cardIDArray[idx] = smartCard.cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - idx - 1];
            }
            ...
            if (cardType == BS2CardTypeEnum.ACCESS)
            {
                ...
                for (int idx = 0; idx < BS2Envirionment.BS2_SMART_CARD_MAX_ACCESS_GROUP_COUNT; ++idx)
                {
        ...
        void print(IntPtr sdkContext, BS2BlackList blacklist)
        {
            Console.WriteLine(">>>> Blacklist issueCount[{0}] cardID[{1}-{2}-{3}-{4}-{5}-{6}-{7}-{8}]",
                                        ...
                                        blacklist.cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 1],
                                        blacklist.cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 2],
                                        blacklist.cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 3],
                                        blacklist.cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 4],
                                        blacklist.cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 5],
                                        blacklist.cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 6],
                                        blacklist.cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 7],
                                        blacklist.cardID[BS2Envirionment.BS2_CARD_DATA_SIZE - 8]);
        }
    }
}
