    ...
    public class AccessControl : FunctionModule
    {
        ...
        public void setAccessGroup(IntPtr sdkContext, uint deviceID, bool isMasterDevice)
        {
            ...
            for (int idx = 0; idx < amount; ++idx)
            {
                ...
                else if (accessGroupName.Length > BS2Envirionment.BS2_MAX_ACCESS_GROUP_NAME_LEN)
                {
                    Console.WriteLine("  Name of access group should less than {0} words.", BS2Envirionment.BS2_MAX_ACCESS_GROUP_NAME_LEN);
                    return;
                }
                else
                {
                    byte[] accessGroupArray = Encoding.UTF8.GetBytes(accessGroupName);
                    Array.Clear(accessGroup.name, 0, BS2Envirionment.BS2_MAX_ACCESS_GROUP_NAME_LEN);
        ...
        public void setAccessLevel(IntPtr sdkContext, uint deviceID, bool isMasterDevice)
        {
            ...
            for (int idx = 0; idx < amount; ++idx)
            {
                ...
                else if (accessGroupName.Length > BS2Envirionment.BS2_MAX_ACCESS_LEVEL_NAME_LEN)
                {
                    Console.WriteLine("  Name of access group should less than {0} words.", BS2Envirionment.BS2_MAX_ACCESS_LEVEL_NAME_LEN);
                    return;
                }
                else
                {
                    byte[] accessGroupArray = Encoding.UTF8.GetBytes(accessGroupName);
                    Array.Clear(accessLevel.name, 0, BS2Envirionment.BS2_MAX_ACCESS_GROUP_NAME_LEN);
                    Array.Copy(accessGroupArray, accessLevel.name, accessGroupArray.Length);
                }

                Console.WriteLine("  How many door schedules do you want to set? [0(default)-{0}]", BS2Envirionment.BS2_MAX_ACCESS_LEVEL_ITEMS);
                Console.Write("  >>>> ");
        ...
        public void setAccessSchedule(IntPtr sdkContext, uint deviceID, bool isMasterDevice)
        {
            ...
            for (int idx = 0; idx < amount; ++idx)
            {
                ...
                else if (accessScheduleName.Length > BS2Envirionment.BS2_MAX_SCHEDULE_NAME_LEN)
                {
                    Console.WriteLine("  Name of access schedule should less than {0} words.", BS2Envirionment.BS2_MAX_SCHEDULE_NAME_LEN);
                    return;
                }
                else
                {
                    byte[] accessScheduleArray = Encoding.UTF8.GetBytes(accessScheduleName);
                    Array.Clear(accessSchedule.name, 0, BS2Envirionment.BS2_MAX_SCHEDULE_NAME_LEN);
                    Array.Copy(accessScheduleArray, accessSchedule.name, accessScheduleArray.Length);
                }

                Console.WriteLine("  How many holiday schedules do you want to set? [0(default)-{0}]", BS2Envirionment.BS2_MAX_HOLIDAY_GROUPS_PER_SCHEDULE);
                Console.Write("  >>>> ");
                ...
                for (byte loop = 0; loop < accessSchedule.numHolidaySchedules; ++loop)
                {
                    ...
                    Console.WriteLine("    How many time period do you want to set? [0(default)-{0}]", BS2Envirionment.BS2_MAX_TIME_PERIODS_PER_DAY);
                    Console.Write("    >>>> ");
                ...
                if (isDailySchedule)
                {
                    ...
                    Console.WriteLine("    How many days is this schedule applied for? [0(default)-{0}]", BS2Envirionment.BS2_MAX_DAYS_PER_DAILY_SCHEDULE);
                    Console.Write("    >>>> ");
                ...
                {
                    for (byte loop = 0; loop < BS2Envirionment.BS2_NUM_WEEKDAYS; ++loop)
                    {
        ...
        public void setHolidayGroup(IntPtr sdkContext, uint deviceID, bool isMasterDevice)
        {
            ...
            for (int idx = 0; idx < amount; ++idx)
            {
                ...
                else if (holidayGroupName.Length > BS2Envirionment.BS2_MAX_HOLIDAY_GROUP_NAME_LEN)
                {
                    Console.WriteLine("  Name of holiday group should less than {0} words.", BS2Envirionment.BS2_MAX_HOLIDAY_GROUP_NAME_LEN);
                    return;
                }
                else
                {
                    byte[] holidayGroupArray = Encoding.UTF8.GetBytes(holidayGroupName);
                    Array.Clear(holidayGroup.name, 0, BS2Envirionment.BS2_MAX_HOLIDAY_GROUP_NAME_LEN);
                    Array.Copy(holidayGroupArray, holidayGroup.name, holidayGroupArray.Length);
                }

                Console.WriteLine("  How many holidays do you want to set? [0(default)-{0}]", BS2Envirionment.BS2_MAX_HOLIDAYS_PER_GROUP);
                Console.Write("  >>>> ");
