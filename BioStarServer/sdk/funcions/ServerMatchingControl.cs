...
namespace Suprema
{
    ...
    abstract class IdentifyUserTask : BaseTask
    {
        ...
        public byte[] templateData = new byte[BS2Envirionment.BS2_FINGER_TEMPLATE_SIZE];
    ...
    class UFMIdentifyUserTask : IdentifyUserTask
    {
        ...
        protected override BS2ErrorCode find(ref BS2UserBlob userBlob)
        {
            ...
            {                
                ...
                byte[] curTemplate = new byte[BS2Envirionment.BS2_FINGER_TEMPLATE_SIZE];
                ...
                while (true)
                {
                    ...
                    {
                        for (int templateIndex = 0; templateIndex < BS2Envirionment.BS2_TEMPLATE_PER_FINGER; ++templateIndex)
                        {
                            Array.Copy(item.Value.data, templateIndex * BS2Envirionment.BS2_FINGER_TEMPLATE_SIZE, curTemplate, 0, BS2Envirionment.BS2_FINGER_TEMPLATE_SIZE);

                            status = matcher.IdentifyNext(curTemplate, BS2Envirionment.BS2_FINGER_TEMPLATE_SIZE, out IdentifySucceed);
    ...
    public class ServerMatchingControl : FunctionModule
    {
        ...
        public void ShowConnectionDeviceList(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            Console.WriteLine("연결된 장치 개수: " + numDevice);
            if (numDevice > 0)
            {
        ...
        public void VerifyUser(UInt32 deviceId, UInt16 seq, byte isCard, byte cardType, IntPtr data, UInt32 dataLen)
        {
            if (Convert.ToBoolean(isCard))
            {
                ...
                Array.Clear(csnCard.data, 0, BS2Envirionment.BS2_CARD_DATA_SIZE);
